...
const handleActions = (e) => {
    ...
    if(e.target.classList.contains('save')){
        ...
        const ageInDays = (Date.now() - Date.parse(inputValues[1]))/(1000 * 60 * 60 * 24);
        ...
        localStorage.setItem('dir', JSON.stringify(directoryData));
    }


    //Delete button
    if(e.target.classList.contains('delete')){
        const localStorageData = JSON.parse(localStorage.getItem('dir'));
...
const retrieveData = () => {
    ...
    const userData = localStorageData.filter(user => user["Aadhar Number"] === inputValue);
    ...
    const informationCard = `
                <div>
                    ...
                    <div>
                        <p>${userData[0]["Name"]}</p>
                        <p>${userData[0]["Date of Birth"]}</p>
                        <p>${userData[0]["Aadhar Number"]}</p>
                        <p>${userData[0]["Mobile Number"]}</p>
                        <p>${userData[0]["Age"]}</p>
                    </div>
                </div>
    `
